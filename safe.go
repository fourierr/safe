...
// returned by Wait.
func (g *Group) Go(f func() error) { g.wg.Add(1) go func() {  defer func() {   if err := recover(); err != nil {    fmt.Println("panic recover ", err)   }  }()  defer g.wg.Done()  if err := f(); err != nil {   g.errOnce.Do(func() {    g.err = err    if g.cancel != nil {     g.cancel()    }   })  } }()}
func Go(f func()) { go func() {  defer func() {   if err := recover(); err != nil {    fmt.Println("panic recover ", err)   }  }()  f() }()}
